...
In this script we will continue to prepare our dataset for use in meta-analysis and calculate variables for effect size estimation (SMDH, lnRR). 
In the previous script [05_data_cleaning.Rmd](https://github.com/shreyadimri/Green_Nest_Material/blob/main/code/05_data_cleaning.Rmd) we merged the data from different sources after data extraction, and performed several data cleaning procedures, including the standardization of terms. We have excluded the articles that did not pass full-text screening and the fitness proxies that we could not find a direction for the effect. The resulting data were saved as [dataset_after_cleaning.csv](https://github.com/shreyadimri/Green_Nest_Material/blob/main/data/03_data_cleaning/dataset_after_cleaning.csv)
...
- [data/04_data_analysis/dataset_analysis.csv]
- [data/04_data_analysis/dataset_lnRR.csv]
- [data/04_data_analysis/dataset_SMDH.csv]
...
* Load cleaned dataset [dataset_after_cleaning.csv](https://github.com/shreyadimri/Green_Nest_Material/blob/main/data/03_data_cleaning/dataset_after_cleaning.csv)
...
## create working copy to store changes
dataset_analysis <- dataset_after_cleaning
```

#### Custom functions

For the presented analyses, several functions are defined, most of them defined in [Nakagawa, Noble, Lagisz, Spake, Viechtbauer, & Senior (2023)](https://doi.org/10.1111/ele.14144) and the accompanying online supplement <https://alistairmcnairsenior.github.io/Miss_SD_Sim/>
...
## Check if the name is specified or not. If not, then assign it the name of the mean, x,
...
##  https://stackoverflow.com/questions/60644445/converting-tidyeval-arguments-to-string
  ...
  ## Calculate between study CV. 
  ## Take weighted mean CV within study, and then take a weighted mean across studies 
  ## of the within study CV. Weighted based on sample size and pooled sample size.
  b_grp_cv_data <- data %>%
...
** However we did not find enough bird species to run a phylogenetic model, so we included only bird species as a random effect **
...
dataset_analysis <- dataset_analysis %>%
  ...
  mutate(experiment_ID_coded = as.factor(paste(paper_ID, experiment_ID, sep="_")),
         .after = experiment_ID) %>%
  mutate(group_ID_coded = as.factor(paste(paper_ID, group_ID, sep="_")),
         .after = group_ID) %>%
  mutate(repeated_trait_ID_coded = as.factor(paste(paper_ID,repeated_trait_ID, sep="_")),
         .after = repeated_trait_ID)
...
```{r cleaning location names}
dataset_analysis <- dataset_analysis %>%
  mutate(population_ID = as.factor(population_location))

# Let's look at the names of the locations
...
For comparisons involving the shared control or treatment groups across multiple studies, we adjust the sample sizes by dividing them by the number of times each group is included in comparisons. This adjustment helps to prevent any inflation of the precision of effect sizes due to repeated use of the same groups and account for the shared-group non-independence. In other words, this adjustment reduces over-precision caused by reusing the same control or treatment group across multiple comparisons.
...
dataset_analysis <- dataset_analysis %>%
  mutate(sd_experiment = if_else(
    condition = type_measure_dispersion_experiment %in% c('se', 'SE'), 
    true = measure_dispersion_experiment * sqrt(as.numeric(n_experiment)), 
    false = measure_dispersion_experiment), 
    .after = measure_dispersion_experiment)

# For control group
dataset_analysis <- dataset_analysis %>%
  mutate(sd_control = if_else(#
    condition = type_measure_dispersion_control %in% c('se', 'SE'), 
    true = measure_dispersion_control* sqrt(as.numeric(n_control)), 
    false = measure_dispersion_control),
...
trait_mean<- dataset_analysis %>%
  group_by(trait_type) %>%
  summarise(cumulative_mean = mean (
...
not_calculated <- dataset_analysis %>% 
  filter(is.na(lnRR)|is.na(lnRR_variance))
...
dataset_analysis <- dataset_analysis %>%
  mutate(lnRR = if_else((
    ...
    lnRR_variance = if_else((
      ...
      v_lnrr_laj,
      lnRR_variance),)%>%
  select(-b_CV2_exp, -b_CV2_con, -cv_Control, -cv_Experimental,
         -cv2_con_new, -cv2_exp_new, -lnrr_laj, -v_lnrr_laj)
...
# and the approximate sampling variance is (n_treatment + n_control) / (n_treatment * n_control).
...
```{r contingency_table}
contingency_table <- dataset_analysis %>%
  filter(proxy_decision == "contingency table")
...
contingency_table <- escalc(
  measure = "OR",
  ai = success_experiment, 
  bi = failure_experiment, 
  ci = success_control, 
  di = failure_control, 
  data = contingency_table,
  var.names = c('lnOR', 'lnOR_variance'))
# Calculate SMDH
contingency_table <- contingency_table %>%
...
```{r}
ancova_SMDH <- dataset_analysis %>% 
  filter(statistics_type=="ANCOVA")
...
dataset_analysis <- rows_update(
  ...
  x = dataset_analysis,           
  y = ancova_SMDH %>% 
    # Updates from ancova_SMDH
    select(Observation_ID, SMDH, SMDH_variance),
  # Match on Observation_ID
  by = "Observation_ID")
```
...
t_test_SMDH <- dataset_analysis %>% 
  filter(statistics_type ==" t-test")
...
```{r, eval = F}
## export to csv file
write.csv(low_sample,
          file = here::here("data/03_data_cleaning/low_sample.csv"), row.names = FALSE)
```
...
```{r}
dataset_analysis <- dataset_analysis %>%
  anti_join(low_sample, by = "Observation_ID")
```

## Saving all output files

Output-writing code is not evaluated when rendering because the generated files are already archived in the repository.

```{r, eval = F}
## Complete dataset that can be used for analysis
write.csv(dataset_analysis, 
          file = here::here("data/04_data_analysis/dataset_analysis.csv"),
          row.names = FALSE)
...
write.csv(dataset_lnRR,
          file = here::here("data/04_data_analysis/dataset_lnRR.csv"),
          row.names = FALSE)
