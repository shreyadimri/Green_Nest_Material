...
## Setting up workspace using pacman
if (!requireNamespace("pacman", quietly = TRUE)) {
  install.packages("pacman")
}

# For plotting trees, and although not strictly necessary, 
# we may need the `ggtree` R package, which you should be able to install by running:
...
## installing orchaRd from GitHub
if (!requireNamespace("orchaRd", quietly = TRUE)) {
  ## installing orchaRd from GitHub
  pacman::p_install_gh("daniel1noble/orchaRd", ref = 'main')
}

## installing ggsankey from GitHub
if (!requireNamespace("ggsankey", quietly = TRUE)) {
  ## installing orchaRd from GitHub
  pacman::p_install_gh("davidsjoberg/ggsankey", ref = 'main')
}
...
For the presented analyses, we need to define the function [sigma2_v](https://github.com/Yefeng0920/heterogeneity_ecoevo/blob/main/function/function.R) to estimate typical sampling error variance
...
These files were created from the code `06_data_preparation.Rmd`
...
dataset_analysis<- dataset_analysis %>%
  mutate(across(c(paper_ID, Observation_ID, repeated_trait_ID_coded,
                  paper_ID, experiment_ID_coded, group_ID_coded,
                  comparision_type, Hypothesis, trait_type,
                  parasite_type, time_of_gnm_addition, blinding,
                  random_assignment, missing_data, population_ID,
                  bird_species),
                as.factor))
...
dataset_SMDH <- dataset_SMDH %>% mutate(across(c(paper_ID, Observation_ID, repeated_trait_ID_coded,
                                                 paper_ID, experiment_ID_coded, group_ID_coded,
                                                 comparision_type, Hypothesis, trait_type,
                                                 parasite_type, time_of_gnm_addition, blinding,
                                                 random_assignment, missing_data, population_ID,
...
lnRR_geary_passed <- dataset_lnRR %>%
...
## selecting only published datasets
...
## selecting dataset without those using 0 as ES 
## because authors reported no effect
...
## selecting dataset which are not flagged 
## but without the SMDH calculated from inferential statistics 
...
VCV_lnRR <- vcalc(
  ...
  obs = Observation_ID,
  data = dataset_lnRR,
  # assuming that the effect sizes within the same study 
  # are correlated with rho = 0.5
  rho = 0.5)

intercept_lnRR <- rma.mv(
  # specify lnRR as the effect size measure;
  yi = lnRR_sign,
  # specify lnRR's sampling variance-covariance matrix;
  V = VCV_lnRR, 
  mods = ~ 1,
  # set random effects structure
...
## saving the model
saveRDS(intercept_lnRR, 
        file = here::here("model/intercept_lnRR.rds"))

## saving the dataset for this model
write.csv(dataset_lnRR,
          here::here("data/04_data_analysis/intercept_lnRR.csv"))
```
...
```{r function-interpret-heterogeneity, eval=FALSE}
# Sun Aug 31 12:01:40 2025 ------------------------------
# Uncommented this code chunk but set eval = F 

## Function is from the Pluralistic approach to heterogeneity 
## from Yefeng's Pre-print repository from GitHub
...
x <- het_interpret(observed_value = I2_main_model_vector[1],
                   ...
                   es_type = "lnRR_sign",
                   data = intercept_lnRR)
# Sun Aug 31 12:04:36 2025 ------------------------------
# Error in het_interpret(observed_value = I2_main_model_vector[1], het_type = "I2", :
# Invalid effect size type. Check the levels of 'es.type'.
```

Let us look at the heterogeneity in lnRR intercept-only model

```{r sigma2_v intercept_lnRR}
# Then also:
...
VCV_SMDH <- vcalc(
  ...
  obs = Observation_ID,
  data = dataset_SMDH,
  # assuming that the effect sizes 
  # within the same study are correlated with rho = 0.5
  rho = 0.5 
)

intercept_SMDH <- rma.mv(
  # specify SMDH as the effect size measure;
  yi = SMDH_sign,
  # specify SMDH's sampling variance-covariance matrix;
  V = VCV_SMDH, 
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
...
```{r Bivariate Meta-analytical model}
dataset_bivariate <- data.frame(
  paper_ID = c(dataset_lnRR$paper_ID, dataset_SMDH$paper_ID),
...
VCV_bivariate <- vcalc(
  # sampling variances of lnRR and SMD that are correlated within the same study;
  vi = V_ES, 
  # study identity;
  cluster = paper_ID,
  # different types of effect size measures underlying the observed effect sizes;
  type = ES_measure, 
  # the long format data frame;
  data = dataset_bivariate, 
  # assuming that the effect sizes within the same study are correlated with rho = 0.5.
  rho = 0.2, 
  # specify whether impose the non positive definite VCV matrix 
  # (not invertible) to the nearest positive semi-definite matrix
  nearpd = TRUE)

Bivariate_model <- rma.mv(
  # specify the effect size estimate (the variable ES in our case);
  yi = ES, 
  # specify the imputed variance-covariance matrix;
  V = VCV_bivariate, 
  # specify the variable "ES_measure" indicating the types of effect size measures;
  mods = ~ ES_measure - 1, 
  ...
  # impose an unstructured variance-covariance of the study-specific random effects;
  struct = "UN",
  # t distribution is specified to test the overall effect against the null hypothesis and construct confidence intervals;
  test = "t", 
  # restricted likelihood maximum is assigned as the estimator for variance components as suggested;
  method = "REML", 
...
# Saving the model
saveRDS(Bivariate_model, 
        file = here::here("model/sensitivity_analysis/bivariate_model.rds"))

# saving the dataset for this model
write.csv(dataset_bivariate,
          here::here("data/05_data_sensitivity_analysis/dataset_bivariate.csv"))
```
...
ab.l <- matreg(
  ...
  R = Bivariate_model$G, 
  cov = TRUE, 
  means = coef(Bivariate_model), 
  n = Bivariate_model$g.levels.comb.k) 

xy <- ellipse(Bivariate_model$G, 
              centre = coef(Bivariate_model),
              # get 95% coverage region
              level = 0.95) 

## convert it into data frame 
## so that we can use ggplot to make a figure
ellipse_df <- data.frame(x = xy[, 1],
                         y = xy[, 2]) 
...
SA_lnRR_published <- rma.mv(
  # specify lnRR as the effect size measure;
  yi = lnRR_sign, 
  # specify lnRR's sampling variance-covariance matrix;
  V = VCV_lnRR_published, 
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(SA_lnRR_published, 
        file = here::here("model/sensitivity_analysis/SA_lnRR_published.rds"))

# saving the dataset for this model
write.csv(dataset_lnRR_published,
          here::here("data/05_data_sensitivity_analysis/dataset_lnRR_published.csv"))
```
...
## Running intercept only model
SA_SMDH_published<- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_published, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
## Saving the model
saveRDS(SA_SMDH_published, 
        file = here::here("model/sensitivity_analysis/SA_SMDH_published.rds"))

## saving the dataset for this model
write.csv(dataset_SMDH_published,
          here::here("data/05_data_sensitivity_analysis/dataset_SMDH_published.csv"))
```
...
## Running intercept only model
SA_lnRR_flagged<- rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
...
## Saving the model
saveRDS(SA_lnRR_flagged, 
        file = here::here("model/sensitivity_analysis/SA_lnRR_flagged.rds"))

## saving the dataset for this model
write.csv(lnRR_con_flagged,
          here::here("data/05_data_sensitivity_analysis/dataset_lnRR_con_flagged.csv"))
```
...
SA_SMDH_flagged<- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_flagged, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
```{r, eval=FALSE}
## Saving the model
saveRDS(SA_SMDH_flagged, 
        file = here::here("model/sensitivity_analysis/SA_SMDH_flagged.rds"))

## saving the dataset for this model
write.csv(SMDH_con_flagged,
          here::here("data/05_data_sensitivity_analysis/dataset_SMDH_con_flagged.csv"))
```
...
# Saving the model
saveRDS(SA_lnRR_no_missingES, 
        file = here::here("model/sensitivity_analysis/SA_lnRR_no_missingES.rds"))

# saving the dataset for this model
write.csv(lnRR_sin_missingES,
          here::here("data/05_data_sensitivity_analysis/dataset_lnRR_sin_missingES.csv"))
```
...
## Running intercept only model
SA_SMDH_no_missingES <- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_no_missingES, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
```{r, eval=FALSE}
# Saving the model
saveRDS(SA_SMDH_no_missingES, 
        file = here::here("model/sensitivity_analysis/SA_SMDH_no_missingES.rds"))
# saving the dataset for this model
write.csv(SMDH_sin_missingES,
          here::here("data/05_data_sensitivity_analysis/dataset_SMDH_sin_missingES.csv"))
```
...
# Preparing new variance-covariance matrix
VCV_flagged_no_missingES <- vcalc(
  vi = lnRR_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
## Running intercept only model
SA_lnRR_flagged_no_missingES<- rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
...
# saving the dataset for this model
write.csv(lnRR_flagged_no_missingES,
          here::here("data/05_data_sensitivity_analysis/dataset_lnRR_flagged_no_missingES.csv"))
```
...
# Preparing new variance-covariance matrix
VCV_SMDH_flagged_no_missingES <- vcalc(
  vi = SMDH_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
## Running intercept only model
SA_SMDH_flagged_no_missingES <- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_flagged_no_missingES, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
```{r, eval=FALSE}
# Saving the model
saveRDS(SA_SMDH_flagged_no_missingES, 
        file = here::here("model/sensitivity_analysis/SA_SMDH_flagged_no_missingES.rds"))

# saving the dataset for this model
write.csv(SMDH_flagged_no_missingES,
          here::here("data/05_data_sensitivity_analysis/dataset_SMDH_flagged_no_missingES.csv"))
```
...
Are the meta-regression robust to this analysis

```{r}
dataset_lnRR_design_flagged_no_missingES <- lnRR_flagged_no_missingES %>% filter(!is.na(comparision_type))

VCV_lnRR_design_flagged_no_missingES <- vcalc(
  vi = lnRR_variance,
  cluster = paper_ID,
  obs = Observation_ID,
  data = dataset_lnRR_design_flagged_no_missingES,
  rho = 0.5 # assuming that the effect sizes within the same study are correlated with rho = 0.5
)

lnRR_design_flagged_no_missingE <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
  V = VCV_lnRR_design_flagged_no_missingES ,# specify lnRR's sampling variance;
...
# This analysis only includes dataset that has a proxy relating to comparison type
dataset_SMDH_design_flagged_no_missingES <- SMDH_flagged_no_missingES %>% filter(!is.na(comparision_type))

VCV_SMDH_design_flagged_no_missingES <- vcalc(
  vi = SMDH_variance,
  cluster = paper_ID,
  obs = Observation_ID,
  data = dataset_SMDH_design_flagged_no_missingES,
  rho = 0.5 # assuming that the effect sizes within the same study are correlated with rho = 0.5
)

SMDH_design_flagged_no_missingE <-
  metafor::rma.mv(
    yi = SMDH_sign, # specify SMDH as the effect size measure;
    V = VCV_SMDH_design_flagged_no_missingES ,# specify SMDH's sampling variance;
    mods = ~ - 1 + comparision_type,
    random = list(~ 1 | paper_ID,
                  ~ 1 | Observation_ID,
                  ~ 1 | group_ID_coded,
...
## Running intercept only model
SA_SMDH_no_inferential<- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_no_inferential, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(SA_SMDH_no_inferential, 
        file = here::here("model/sensitivity_analysis/SA_SMDH_no_inferential.rds"))

# saving the dataset for this model
write.csv(SMDH_no_inferential,
          here::here("data/05_data_sensitivity_analysis/dataset_SMDH_no_inferential.csv"))
```
...
```{r, eval=FALSE}
# Saving the model
saveRDS(SA_lnRR_geary_passed, 
        file = here::here("model/sensitivity_analysis/SA_lnRR_geary_passed.rds"))

# saving the dataset for this model
write.csv(lnRR_geary_passed,
          here::here("data/05_data_sensitivity_analysis/dataset_lnRR_geary_passed.csv"))
```
...
SA_lnRR_no_para <- rma.mv(
  yi = lnRR_sign, # specify SMDH as the effect size measure;
  V = VCV_lnRR_no_para, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(SA_lnRR_no_para, 
        file = here::here("model/sensitivity_analysis/SA_lnRR_no_para.rds"))

# saving the dataset for this model
write.csv(dataset_lnRR_no_para,
          here::here("data/05_data_sensitivity_analysis/dataset_lnRR_no_para.csv"))
```
...
# for  SMDH  as Effect Size
#----------------------------------------------------------------------------
dataset_SMDH_no_para <- dataset_SMDH %>%
  filter(is.na(parasite_type))

## The dataset is reduced to almost half
nrow(dataset_SMDH) - nrow(dataset_SMDH_no_para)

VCV_SMDH_no_para <- vcalc(
  vi = SMDH_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
SA_SMDH_no_para<- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_no_para, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
```{r, eval=FALSE}
# Saving the model
saveRDS(SA_SMDH_no_para, 
        file = here::here("model/sensitivity_analysis/SA_SMDH_no_para.rds"))

# saving the dataset for this model
write.csv(dataset_SMDH_no_para,
          here::here("data/05_data_sensitivity_analysis/dataset_SMDH_no_para.csv"))
```
...
```{r lnRR rho sensitivity analysis, eval=FALSE}
# Sun Aug 31 13:13:02 2025 ------------------------------
# Make Code chunk executable ...
rho_range <- c(0.2,0.3, 0.5, 0.7,0.8)

# repeatedly run the specified model with varying rho
SA_lnRR_VCV_range <- list() 
for (i in 1:length(rho_range)) {
  ...
  # Sun Aug 31 13:13:25 2025 ------------------------------
  #set lnRR_VCV_range[[i]] to SA_lnRR_VCV_range[[i]]
  
  SA_lnRR_VCV_range[[i]] <- rma.mv(
    yi = lnRR_sign,
    V = VCV_range,
    random = list(~ 1 | paper_ID,
                  ~ 1 | Observation_ID,
...
save(SA_SMDH_VCV_range, 
     file = here::here("model/sensitivity_analysis/SA_SMDH_VCV_range.rds"))
```
...
## -----------------------------------------------------------
# Save model
saveRDS(lnRR_hypothesis, 
        file = here::here("model/lnRR_hypothesis.rds"))

# saving the dataset for this model
write.csv(dataset_lnRR,
          here::here("data/04_data_analysis/hypothesis_lnRR.csv"))
```
...
SMDH_hypothesis <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH,# specify SMDH's sampling variance;
  mods = ~ - 1 + Hypothesis,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
```{r, eval=FALSE}
# Save model
saveRDS(SMDH_hypothesis,
        file = here::here("model/SMDH_hypothesis.rds"))

# saving the dataset for this model
write.csv(dataset_SMDH,
          here::here("data/04_data_analysis/hypothesis_SMDH.csv"))
```
...
# Saving the model
saveRDS(lnRR_design,
        file = here::here("model/lnRR_design.rds"))

# saving the dataset for this model
write.csv(dataset_lnRR_design,
          here::here("data/04_data_analysis/design_lnRR.csv"))
```
...
SMDH_design <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_design ,# specify SMDH's sampling variance;
  mods = ~ - 1 + comparision_type,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(SMDH_design, 
        file = here::here("model/SMDH_design.rds"))

# saving the dataset for this model
write.csv(dataset_SMDH_design,
          here::here("data/04_data_analysis/design_SMDH.csv"))
```
...
dataset_lnRR_no_comp3 <- dataset_lnRR %>% 
  filter(comparision_type!=3 | is.na(comparision_type))

## Only some rows are removed... 
nrow(dataset_lnRR) - nrow(dataset_lnRR_no_comp3)

VCV_lnRR_no_comp3 <- vcalc(
  vi = lnRR_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
SA_lnRR_no_comp3<- rma.mv(
  yi = lnRR_sign, # specify SMDH as the effect size measure;
  V = VCV_lnRR_no_comp3, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
```{r, eval=FALSE}
# Saving the model
saveRDS(SA_lnRR_no_comp3, 
        file = here::here("model/sensitivity_analysis/SA_lnRR_no_comp3.rds"))

# saving the dataset for this model
write.csv(dataset_lnRR_no_comp3,
          here::here("data/05_data_sensitivity_analysis/dataset_lnRR_no_comp3.csv"))
```
...
dataset_SMDH_no_comp3 <- dataset_SMDH %>%
  filter(comparision_type!=3 | is.na(comparision_type))

## Only some rows are removed... 
nrow(dataset_SMDH) - nrow(dataset_SMDH_no_comp3)

VCV_SMDH_no_comp3 <- vcalc(
  vi = SMDH_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
SA_SMDH_no_comp3<- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_no_comp3, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(SA_SMDH_no_comp3, 
        file = here::here("model/sensitivity_analysis/SA_SMDH_no_comp3.rds"))

# saving the dataset for this model
write.csv(dataset_SMDH_no_comp3,
          here::here("data/05_data_sensitivity_analysis/dataset_SMDH_no_comp3.csv"))
```
...
```{r}
dataset_lnRR_design_SA <- dataset_lnRR %>%
  filter(comparision_type!=3)

VCV_lnRR_design_SA <- vcalc(
  vi = lnRR_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
dataset_SMDH_design_SA <- dataset_SMDH %>%
  filter(comparision_type!=3)

VCV_SMDH_design_SA <- vcalc(
  vi = SMDH_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
SMDH_design_SA <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_design_SA ,# specify SMDH's sampling variance;
  mods = ~ - 1 + comparision_type,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
lnRR_time_gnm_addition <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
...
## Saving the model
saveRDS(lnRR_time_gnm_addition, 
        file = here::here("model/lnRR_time_gnm_addition.rds"))

# saving the dataset for this model
write.csv(dataset_lnRR,
          here::here("data/04_data_analysis/time_gnm_lnRR.csv"))
```

```{r}
## Now similarly for the SMDH effect size using the complete dataset
SMDH_time_gnm_addition <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH,# specify SMDH's sampling variance using the variance-covariance matrix we created for the intercept-only model since the whole dataset is being used in this case as well
  mods = ~ - 1 + time_of_gnm_addition,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
## Saving the model
saveRDS(SMDH_time_gnm_addition, 
        file = here::here("model/SMDH_time_gnm_addition.rds"))

# saving the dataset for this model
write.csv(dataset_SMDH,
          here::here("data/04_data_analysis/time_gnm_SMDH.csv"))
```
...
# Saving the model
saveRDS(lnRR_parasite, 
        file = here::here("model/lnRR_parasite.rds"))

# saving the dataset for this model
write.csv(dataset_lnRR_parasite,
          here::here("data/04_data_analysis/parasite_lnRR.csv"))
```

```{r}
### Similarly for SMDH as well selecting the dataset with parasite type
# This analysis only includes dataset that has a proxy relating to parasite or pathogen
dataset_SMDH_parasite <- dataset_SMDH %>%
  filter(!is.na(parasite_type))

VCV_SMDH_parasite <- vcalc(
  vi = SMDH_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
SMDH_parasite <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_parasite,# specify SMDH's sampling variance;
  mods = ~ - 1 + parasite_type,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Sensitivity Analysis: Excluding Non-aromatic Vs. Aromatic

We are aware that removing over 50% of the dataset will cause some moderator levels to drop out. But if our goal is to critically examine the role of aromaticity and potentially argue that its importance has been overstated, this could be a very informative step.

```{r}
## For the lnRR effect size...
dataset_lnRR_no_comp1<-dataset_lnRR %>%
  filter(comparision_type!=1 | is.na(comparision_type))

## Only some rows are removed... 
nrow(dataset_lnRR) - nrow(dataset_lnRR_no_comp1)

VCV_lnRR_no_comp1 <- vcalc(
  vi = lnRR_variance,
  cluster = paper_ID,
  obs = Observation_ID,
  data = dataset_lnRR_no_comp1,
  rho = 0.5 # assuming that the effect sizes within the same study are correlated with rho = 0.5
)

SA_lnRR_no_comp1<- rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
  V = VCV_lnRR_no_comp1, # specify lnRR's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(SA_lnRR_no_comp1, file = here::here("model/sensitivity_analysis/SA_lnRR_no_comp1.rds"))

# saving the dataset for this model
write.csv(dataset_lnRR_no_comp1,here::here("data/05_data_sensitivity_analysis/dataset_lnRR_no_comp1.csv"))
```
...
dataset_SMDH_no_comp1 <- dataset_SMDH %>%
  filter(comparision_type!=1 | is.na(comparision_type))

## Only some rows are removed... 
nrow(dataset_SMDH) - nrow(dataset_SMDH_no_comp1)

VCV_SMDH_no_comp1 <- vcalc(
  vi = SMDH_variance,
  cluster = paper_ID,
  obs = Observation_ID,
  data = dataset_SMDH_no_comp1,
  rho = 0.5 # assuming that the effect sizes within the same study are correlated with rho = 0.5
)

SA_SMDH_no_comp1<- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_no_comp1, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ 1,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(SA_SMDH_no_comp1, file = here::here("model/sensitivity_analysis/SA_SMDH_no_comp1.rds"))

# saving the dataset for this model
write.csv(dataset_SMDH_no_comp1,here::here("data/05_data_sensitivity_analysis/dataset_SMDH_no_comp1.csv"))
```

```{r}
#----------------------------------------------------------------------------
# Rerunning the two mechanistic hypothesis meta-regressions with those effect sizes excluded
#----------------------------------------------------------------------------

lnRR_hypothesis_no_comp1 <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
  V = VCV_lnRR_no_comp1,# specify lnRR's sampling variance;
  mods = ~ - 1 + Hypothesis,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Save model
saveRDS(lnRR_hypothesis_no_comp1, file = here::here("model/lnRR_hypothesis_no_comp1.rds"))
```

```{r}
SMDH_hypothesis_no_comp1 <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_no_comp1,# specify SMDH's sampling variance;
  mods = ~ - 1 + Hypothesis,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Save model
saveRDS(SMDH_hypothesis_no_comp1, file = here::here("model/SMDH_hypothesis_no_comp1.rds"))
```

```{r}
#----------------------------------------------------------------------------
## Now re-running the meta-regressions once again with experimental design in it
#----------------------------------------------------------------------------

# This analysis only includes dataset that has a proxy relating to comparison type
dataset_lnRR_design_no_comp1 <- dataset_lnRR_no_comp1 %>%
  filter(!is.na(comparision_type))

VCV_lnRR_design_no_comp1 <- vcalc(
  vi = lnRR_variance,
  cluster = paper_ID,
  obs = Observation_ID,
  data = dataset_lnRR_design_no_comp1,
  rho = 0.5 # assuming that the effect sizes within the same study are correlated with rho = 0.5
)
lnRR_design_no_comp1 <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
  V = VCV_lnRR_design_no_comp1 ,# specify lnRR's sampling variance;
  mods = ~ - 1 + comparision_type,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(lnRR_design_no_comp1, file = here::here("model/lnRR_design_no_comp1.rds"))
```

```{r}
# This analysis only includes dataset that has a proxy relating to comparison type
dataset_SMDH_design_no_comp1 <- dataset_SMDH_no_comp1 %>%
  filter(!is.na(comparision_type))

VCV_SMDH_design_no_comp1 <- vcalc(
  vi = SMDH_variance,
  cluster = paper_ID,
  obs = Observation_ID,
  data = dataset_SMDH_design_no_comp1,
  rho = 0.5 # assuming that the effect sizes within the same study are correlated with rho = 0.5
)

SMDH_design_no_comp1 <- metafor::rma.mv(
  yi = lnRR_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_design_no_comp1 ,# specify SMDH's sampling variance;
  mods = ~ - 1 + comparision_type,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
# Saving the model
saveRDS(SMDH_design_no_comp1, file = here::here("model/SMDH_design_no_comp1.rds"))
```

# Risk of Bias

We have pre-registered the Risk Of Bias analysis, the three we will explore:

1.  **Blinding**

```{r}
unique(dataset_lnRR$blinding)

lnRR_blinding <- metafor::rma.mv(
  yi = lnRR_sign,
  V = VCV_lnRR,
  mods = ~ - 1 + blinding,
  random = list(~ 1 | paper_ID,
...
# Saving the model
saveRDS(lnRR_blinding, file = here::here("model/lnRR_blinding.rds"))

# Saving the dataset for this model
write.csv(dataset_lnRR,here::here("data/04_data_analysis/lnRR_blinding.csv"))
```

```{r}
SMDH_blinding <- metafor::rma.mv(
  yi = SMDH_sign, 
  V = VCV_SMDH,
  mods = ~ - 1 + blinding,
  random = list(~ 1 | paper_ID,
...
# Saving the model
saveRDS(SMDH_blinding, file = here::here("model/SMDH_blinding.rds"))

# Saving the dataset for this model
write.csv(dataset_SMDH,here::here("data/04_data_analysis/SMDH_blinding.csv"))
```

2.  Random Assignment

```{r}
unique(dataset_lnRR$random_assignment)

lnRR_random_assignment <- metafor::rma.mv(
  yi = lnRR_sign,
  V = VCV_lnRR,
  mods = ~ - 1 + random_assignment,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
                ~ 1 | population_ID,
...
# Saving the model
saveRDS(lnRR_random_assignment, file = here::here("model/lnRR_random_assignment.rds"))

# Saving the dataset for this model
write.csv(dataset_lnRR,here::here("data/04_data_analysis/lnRR_random_assignment.csv"))
```

```{r}
SMDH_random_assignment <- metafor::rma.mv(
  yi = SMDH_sign, 
  V = VCV_SMDH,
  mods = ~ - 1 + random_assignment,
  random = list(~ 1 | paper_ID,
...
# Saving the model
saveRDS(SMDH_random_assignment, file = here::here("model/SMDH_random_assignment.rds"))

# Saving the dataset for this model
write.csv(dataset_SMDH,here::here("data/04_data_analysis/SMDH_random_assignment.csv"))
```

3.  Missing Vs. Not Missing

```{r}
unique(dataset_lnRR$missing_data)

lnRR_missing_data <- metafor::rma.mv(
  yi = lnRR_sign,
  V = VCV_lnRR,
  mods = ~ - 1 + missing_data,
  random = list(~ 1 | paper_ID,
...
# Saving the model
saveRDS(lnRR_missing_data, file = here::here("model/lnRR_missing_data.rds"))

# Saving the dataset for this model
write.csv(dataset_lnRR,here::here("data/04_data_analysis/lnRR_missing_data.csv"))
```

```{r}
SMDH_missing_data <- metafor::rma.mv(
  yi = SMDH_sign, 
  V = VCV_SMDH,
  mods = ~ - 1 + missing_data,
  random = list(~ 1 | paper_ID,
...
# Saving the model
saveRDS(SMDH_missing_data, file = here::here("model/SMDH_missing_data.rds"))

# Saving the dataset for this model
write.csv(dataset_SMDH,here::here("data/04_data_analysis/SMDH_missing_data.csv"))
```
...
dataset_lnRR_published <- dataset_lnRR_published %>%
  mutate(effective_n_inv = 
           1/((effective_n_experiment*effective_n_control)/(effective_n_experiment+effective_n_control))) %>%
  mutate(sqrt_effective_n_inv=sqrt(effective_n_inv))

lnRR_small_study <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
...
SMDH_small_study <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_published,# specify SMDH's sampling variance;
  mods = ~sqrt_effective_n_inv,
  random = list(~ 1 | paper_ID,
                ~ 1 | Observation_ID,
                ~ 1 | group_ID_coded,
...
SMDH_small_study2 <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH,# specify SMDH's sampling variance;
  mods = ~sqrt_effective_n_inv,
  random = list(~ 1 | paper_ID,
...
summary(SMDH_small_study2)

# saving the model
saveRDS(SMDH_small_study2, file = here::here("model/SMDH_small_study2.rds"))
```
...
lnRR_decline_eff <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
  V = VCV_lnRR_published,# specify lnRR's sampling variance;
...
SMDH_decline_eff <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_published,# specify SMDH's sampling variance;
  mods = ~ year_MeanCenter,
  random = list(~ 1 | paper_ID,
...
# 
dataset_lnRR_all_in <- dataset_lnRR %>%
  mutate(year_publication = case_when(is.na(year_publication) ~ 2025, TRUE ~ year_publication)) %>%
  mutate(year_MeanCenter = scale(as.numeric(year_publication), scale = F)[,1]) %>%
  mutate(effective_n_inv = 
           1/((4*effective_n_experiment * effective_n_control)/(effective_n_experiment + effective_n_control))) %>%
  mutate(sqrt_effective_n_inv = sqrt(effective_n_inv)) %>%
  mutate(comparision_type = case_when(is.na(comparision_type) ~ as.factor(4), TRUE ~ comparision_type))

VCV_lnRR_all_in <- vcalc(
  vi = lnRR_variance,
  cluster = paper_ID,
  obs = Observation_ID,
...
# saving the model
saveRDS(lnRR_all_in, file = here::here("model/lnRR_all_in.rds"))
```

```{r}
## For SMDH complete published dataset

dataset_SMDH_all_in <- dataset_SMDH %>%
  mutate(year_publication = case_when(is.na(year_publication) ~ 2025, TRUE ~ year_publication)) %>%
  mutate(year_MeanCenter = scale(as.numeric(year_publication), scale = F)[,1]) %>%
  mutate(effective_n_inv = 
           1/((4 * effective_n_experiment * effective_n_control)/(effective_n_experiment + effective_n_control))) %>%
...
SMDH_all_in <- rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_all_in, # specify SMDH's sampling variance-covariance matrix;
  mods = ~ time_of_gnm_addition +
    year_MeanCenter +
    sqrt_effective_n_inv +
...
# saving the model
saveRDS(SMDH_all_in, file = here::here("model/SMDH_all_in.rds"))
```

# Pair-wise comparison for meta-regression

Including post-hoc Wald tests

For this I have to run all the meta-regressions again without removing the intercepts
...
lnRR_hypothesis_pc <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
  V = VCV_lnRR,# specify lnRR's sampling variance;
  mods = ~ Hypothesis,
  random = list(~ 1 | paper_ID,
...
# I now want to test for statistical differences between groups. I need to do Post-hoc Wald tests:
...
SMDH_hypothesis_pc <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH,# specify SMDH's sampling variance;
  mods = ~ Hypothesis,
  random = list(~ 1 | paper_ID,
...
## We use the complete dataset in this case
lnRR_time_gnm_addition_pc <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
  V = VCV_lnRR,# specify lnRR's sampling variance using the variance-covariance matrix we created for the intercept-only model since the whole dataset is being used in this case as well
  mods = ~ time_of_gnm_addition,
  random = list(~ 1 | paper_ID,
...
```{r}
## Now similarly for the SMDH effect size using the complete dataset
SMDH_time_gnm_addition_pc <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH,# specify SMDH's sampling variance using the variance-covariance matrix we created for the intercept-only model since the whole dataset is being used in this case as well
  mods = ~ time_of_gnm_addition,
  random = list(~ 1 | paper_ID,
...
lnRR_parasite_pc <- metafor::rma.mv(
  yi = lnRR_sign, # specify lnRR as the effect size measure;
  V = VCV_lnRR_parasite,# specify lnRR's sampling variance;
  mods = ~ parasite_type,
  random = list(~ 1 | paper_ID,
...
SMDH_parasite_pc <- metafor::rma.mv(
  yi = SMDH_sign, # specify SMDH as the effect size measure;
  V = VCV_SMDH_parasite,# specify SMDH's sampling variance;
  mods = ~ parasite_type,
  random = list(~ 1 | paper_ID,
