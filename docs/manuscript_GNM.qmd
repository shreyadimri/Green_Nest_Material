...
Most bird species, showed a small positive non-significant increase it fitness with an experimental increase in green nest material, although some species showed negative but similarly non-significant effects \[[\@figsupp-MR](C-D); Table X\]. All confidence and prediction intervals overlapped zero. However, bird species identity explained a proportion of heterogeneity in the effect sizes ($R^2_{marginal(lnRR)}$= `{r} as.double(round(orchaRd::r2_ml(lnRR_birds)*100,1))[1]`% and $R^2_{marginal(SMDH)}$= `{r} as.double(round(orchaRd::r2_ml(SMDH_birds)*100,1))[1]`%).
