...
**Mechanisms behind green nest material effects**
...
Using the multilevel meta-regressions with hypothesis type as a moderator, we found no clear support for either of the proposed mechanism [@fig-hypothesis]. There was a small positive but non-significant effect of green nest material on reproductive success in parents (courtship hypothesis) \[lnRR~courtship~ = `{r} as.double(formatC(coef(lnRR_hypothesis)[2], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(lnRR_hypothesis$pval[2], digits = 3, format = "f"))` and SMDH~courtship~ = `{r} as.double(formatC(coef(SMDH_hypothesis)[2], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(SMDH_hypothesis$pval[2], digits = 3, format = "f"))`\]. We found even smaller, non-significant effect of green nest material on protective effect or overall health benefits for the nestling as suggested by nest protection and drug hypothesis \[lnRR~parental_care~ = `{r} as.double(formatC(coef(lnRR_hypothesis)[3], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(lnRR_hypothesis$pval[3], digits = 3, format = "f"))` and SMDH~parental_care~ = `{r} as.double(formatC(coef(SMDH_hypothesis)[3], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(SMDH_hypothesis$pval[3], digits = 3, format = "f"))`\]. In case of the third category, both, that included the fitness proxies that could not be clearly assigned to either mechanisms, we found a small positive effect \[lnRR~both~ = `{r} as.double(formatC(coef(lnRR_hypothesis)[1], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(lnRR_hypothesis$pval[1], digits = 3, format = "f"))` and SMDH~both~ = `{r} as.double(formatC(coef(SMDH_hypothesis)[1], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(SMDH_hypothesis$pval[1], digits = 3, format = "f"))`\]. The moderator explained very little heterogeneity ($R^2~marginal$ for lnRR = `{r} as.double(round(orchaRd::r2_ml(lnRR_hypothesis)*100,1))[1]`% and ($R^2~marginal$ for SMDH = `{r} as.double(round(orchaRd::r2_ml(SMDH_hypothesis)*100,1))[1]`%).
...
There was no evidence that the effect of green nest material on parasite or pathogen load differed between arthropods and micro-organisms \[[@supp-fig-MR-non-sig]A-B; lnRR~arthropods~ = `{r} as.double(formatC(coef(lnRR_parasite)[1], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(lnRR_parasite$pval[1], digits = 3, format = "f"))` and SMDH~arthropods~ = `{r} as.double(formatC(coef(SMDH_parasite)[1], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(SMDH_parasite$pval[1], digits = 3, format = "f"))`; lnRR~micro-organisms~ = `{r} as.double(formatC(coef(lnRR_parasite)[2], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(lnRR_parasite$pval[2], digits = 3, format = "f"))` and SMDH~micro-organisms~ = `{r} as.double(formatC(coef(SMDH_parasite)[2], digit = 3, format="f"))` , p-value = `{r} as.double(formatC(SMDH_parasite$pval[2], digits = 3, format = "f"))`\]. The type of parasite or pathogen explained very little heterogeneity ($R^2~marginal$ for lnRR = `{r} as.double(round(orchaRd::r2_ml(lnRR_parasite)*100,1))[1]`% and $R^2~marginal$ for SMDH = `{r} as.double(round(orchaRd::r2_ml(SMDH_parasite)*100,1))[1]`%).

The meta-regression using bird species as a moderator showed no significantly different effect of green nest material on fitness depending on the bird being studied \[[@supp-fig-MR-non-sig]C-D\]; . However, it explained some heterogeneity in the effect sizes ($R^2~marginal$ for lnRR = `{r} as.double(round(orchaRd::r2_ml(lnRR_birds)*100,1))[1]`% and $R^2~marginal$ for SMDH = `{r} as.double(round(orchaRd::r2_ml(SMDH_birds)*100,1))[1]`%).

We explored the effect of green nest material on fitness depending on the type of trait studies (levels:physiology, morphology, reproduction, behaviour, parasite and pathogenic load, phenology) using a meta-regression with the trait type as a moderator.

To test the effect of time of addition of green nest material on the fitness outcome (levels: before egg hatching, after egg hatching, continuously throughout the nesting phase) we ran a meta-regression with Time of addition of green nest material as the moderator.

We explored the impact of methodological differences on the relationship between green nest material and fitness. To understand the effect of type of experimental design (i.e. whether the authors compared - non-aromatic vs. aromatic, no added material vs. aromatic, no added material vs. non-aromatic), we ran a meta-regression with type of experimental design as the moderator.
...
![](figures/Figure_parasite_birds_trait.png){#supp-fig-MR-non-sig}
