...
geographic_distribution <- ggdraw() +
  draw_plot(fig_map) +
  draw_plot(zoom_map, x = 0.10, y = 0.12, width = 0.52, height = 0.22) + theme(panel.border = element_rect(colour = "black", fill=NA, size=1.5))
...
intercept_SMDH_plot <- orchaRd::orchard_plot(intercept_SMDH,
                                    ...
                                    twig.size = 0.5,
                                    alpha = 0.2,
                                    xlab = "Effect size (Standardized Mean Difference- SMD(H))",
                                    fill = TRUE) +
...
SMDH_hypothesis_plot <-
  ...
  labs(x = "",
    ...
    axis.title = element_text(size = 10),
    axis.text.x = element_text(size = 10),
    axis.text.y = element_text(angle = 90, vjust = 0.8, hjust = 1)
  )+ scale_x_discrete(labels = c("Both","Courtship","Parental Care"))
